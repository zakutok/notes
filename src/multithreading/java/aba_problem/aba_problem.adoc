...
Тут перший потік не може визначити що ресурс модифікувався іншим потоком який спочатку змінює стан ресурсу, а потім повертає ресурс в початковий стан.
...
Наступна діаграма зображає цю проблему на прикладі реалізації наївної версії алгоритму без блокування (CAS алгоритм) роботи зв'язаного списку:
