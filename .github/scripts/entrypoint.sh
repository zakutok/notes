...
echo "Configure git"
/usr/sbin/apk add openssh-client -q > /dev/null
...
if [[ $INPUT_SLIDES_SKIP_ASCIIDOCTOR_BUILD == false ]]; then
    ...
    find . -name "*$INPUT_ADOC_FILE_EXT" -exec asciidoctor -r asciidoctor-multipage -b multipage_html5 $INPUT_ASCIIDOCTOR_PARAMS {} \;
